...
def create_step():
    ...
    current_tags = []
    actions_for_layout = convert_collection_to_string_using_variable(
        collection=actions, variable_name="text"
    )
    todo_actions_for_layout = convert_collection_to_string_using_method(
        collection=todo_actions, method_name="format_for_todo_listbox"
    )
    layout = step_creation.get_layout(
        domain,
        login_path,
        last_site,
        username_field,
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == "Exit":
            ...
            sg.popup_notify(
                "Also saved your configuration", title="Bye bye :D", icon=OK_PNG
            )
            break
            ...
            actions_choice_select.find_all_selections(
                window, values, event, site_info, current_tags
            )
            ...
            tag_list_listbox.pick_tag_in_tags(
                window=window,
            ...
            save_configuration_button.save_configuration(
                window, values, event, todo_actions
            )
        elif event == "-LOAD-CONFIGURATION-":
            load_configuration_button.load_configuration(
                window, values, event, todo_actions
            )
        elif event == "-GENERATE-PLAN-":
            generate_plan_button.generate_plan(
                window, todo_actions, plan_is_an_action=False
            )
        elif event == "-GENERATE-PLAN-AS-ACTION-":
            generate_plan_button.generate_plan(
                window, todo_actions, plan_is_an_action=True
            )
            reload_actions_button.reload(window, imported_actions)
            ...
            bdd_attribute_select.change_bdd_attribute(
                values,
                event,
                window,
                given_when_actions,
            ...
            move_action_arrows_buttons.move_action(
                event=event,
                window=window,
                sg=sg,
                error_picture=ERROR_PNG,
                todo_actions=todo_actions,
            )
            ...
            save_changed_xpath_button.save_changed(
                window=window, todo_actions=todo_actions
            )
        elif event == "-XPATH-EXISTS-":
            xpath_viewer_input.check_if_xpath_unique(
                last_used_html=site_info.get_last_used_html(),
                window=window,
                todo_actions=todo_actions,
            )
        elif event == "-ACTION-LIST-":
            todo_actions_listbox.pick_tag_in_actions(
                window=window, todo_actions=todo_actions
            )
            todo_actions_and_tag_listboxes.only_one_selected(event, window)
            ...
            autocomplete_input.autocomplete(
                values, window, urls_from_project, layout_key=event
            )
