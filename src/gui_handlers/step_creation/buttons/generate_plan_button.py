...
def generate_plan(window, todo_actions, plan_is_an_action):
    ...
    index_found, file_of_found_index = check_if_step_is_already_defined(
        bdd_attribute.lower(), test_title
    )
    whole_plan = actions_to_steps_transform.create(
        todo_actions, test_title, bdd_attribute.lower()
    )
