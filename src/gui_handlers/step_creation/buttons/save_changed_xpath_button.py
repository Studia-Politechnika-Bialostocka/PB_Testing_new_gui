...
def save_changed(window, todo_actions):
    ...
    if len(index) == 0:
        sg.popup_notify(
            "Nothing selected in actions", icon=ERROR_PNG, title="Not permitted"
        )
        return
    ...
    if todo_action.element_html is None:
        sg.popup_notify(
            "Selected action doesn't include xpath",
            icon=ERROR_PNG,
            title="Not permitted",
        )
    ...
    sg.popup_notify(
        "Saved attributes to selected action", icon=OK_PNG, title="Saved xpath"
    )
