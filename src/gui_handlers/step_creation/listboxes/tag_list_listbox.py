...
def pick_tag_in_tags(window, current_tags, last_used_html, todo_actions):
    ...
    if len(index) == 0:
        sg.popup_notify(
            "Click on select next to label Available actions for your plan and find your desirable tag to find",
            icon=ERROR_PNG,
    ...
    if len(current_tags) != 0:
        ...
        xpath_viewer_input.check_if_xpath_unique(
            last_used_html=last_used_html, window=window, todo_actions=todo_actions
        )
