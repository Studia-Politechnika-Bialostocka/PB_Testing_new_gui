...
def find_all_selections(window, values, event, site_info, current_tags):
    ...
    if values[event] in [
        "visiting site",
        "waiting for amount of seconds",
        "use saved actions",
        "assert url of site",
        "assert title of site",
    ]:
        tags_found = []
        need_to_search_for_tags = False

    elif values[event] in [
        "filling input",
        "clicking input",
    ...
    if need_to_search_for_tags is True:
        if values["-LOGGED-IN-"]:
            tags_found, thrown_exception = site_info.get_tag_logged_in(
                site,
                username_field,
                username_value,
                password_field,
            ...
            tags_found, thrown_exception = site_info.get_tag_anonymous(
                site, tag=tag, tag_attributes=tag_attributes
            )
    ...
    listed = list(
        map(lambda obj: obj.format_for_listbox_with_available_actions(), tags_found)
    )
