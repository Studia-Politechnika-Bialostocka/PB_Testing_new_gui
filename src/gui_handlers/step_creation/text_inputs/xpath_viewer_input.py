...
def check_if_xpath_unique(last_used_html, window, todo_actions):
    ...
    if action_list_checked:
        ...
        matches, matches_number = xpath_check.get_number_of_match(
            html=todo_action_checked.site_html, xpath=xpath
        )
    else:
        matches, matches_number = xpath_check.get_number_of_match(
            html=last_used_html, xpath=xpath
        )
    ...
    if matches_number == 1:
        ...
        sg.popup_notify(
            "Fix it and try again", title="Xpath is not correct", icon=ERROR_PNG
        )
        return
    else:
        window["-XPATH-INPUT-"].update(background_color="#ff3e19")

    window["-XPATH-ELEMENTS-"].update(
        f"Found {matches_number} tags matching this xpath"
    )
