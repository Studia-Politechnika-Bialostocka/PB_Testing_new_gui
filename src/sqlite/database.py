...
def setup():
    ...
    table_with_original_urls_exists = c.execute(
        """
        SELECT name FROM sqlite_master WHERE type='table' AND name='original_urls'; """
    ).fetchall()

    table_with_user_data_exists = c.execute(
        """
            SELECT name FROM sqlite_master WHERE type='table' AND name='urls_and_attributes'; """
    ).fetchall()
    ...
    if not table_with_user_data_exists:
        ...
        c.execute(
            """CREATE TABLE urls_and_attributes
                                (url VARCHAR)"""
        )
        conn.commit()
    else:
